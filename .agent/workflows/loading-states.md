...
- **Loading** = the user is waiting on a fetch or mutation. Use `Spinner`
  ...
  blocking actions.
- **Status** = the system is reporting its own state (AI is mid-thought,
  socket is reconnecting, toast is pending). Pulsing-dot and colored-dot
  indicators are status, not loading. **Do not replace them with a
  Spinner.**

Concrete preserved cases:

- AI typing-indicator dots in `pages/ai-assistant/PortalMessageItem.tsx`
- Connection-health pulsing dots in `lib/chat-sdk/ui/SessionChat.tsx`,
  `lib/chat-sdk/ui/DockedChatPanel.tsx`, `pages/ai-assistant/AiAssistantPage.tsx`
- `Loader2Icon` in `components/ui/default/sonner.tsx` (vendored shadcn)
...
- (no color class) — inside a button or colored container that already
  sets text color.
- `text-brand-progressive` — chat-UX accent for AI-related elements
  (allowed exception, confined to `pages/ai-assistant/` and
  `lib/chat-sdk/`).
...
<Empty>
  <EmptyHeader>
    ...
    <EmptyTitle>목적모델을 불러오지 못했습니다</EmptyTitle>
    <EmptyDescription>
...
Reference implementations: `pages/vehicle-view/VehicleViewPage.tsx`,
`pages/analysis-plan/AnalysisPlanListPage.tsx`,
`pages/analysis-plan/AnalysisPlanDetailPage.tsx`.
...
version badge, a count). When the absence of that data has a
sensible visual fallback (`—`, `0`, hidden), no loading or error UI is
required at the layout level — the primary content (`Outlet`,
children) handles its own state.

Example: `pages/parameter-data-creation/ParameterDataCreationLayout.tsx`
fetches a sheet list only to derive a version badge; the badge falls
back to `—`.
